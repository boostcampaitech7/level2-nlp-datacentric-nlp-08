...
EXAONE_FEW_SHOT_TEMP = '''[|system|] You are EXAONE model from LG AI Research, a helpful assistant. [|endofturn|]
[|user|] 다음 헤드라인에 빠진 부분을 올바른 단어로 복원하고 문맥에 맞춰 헤드라인을 작성해줘.

### 다음 지침을 따라야해.
- 제시된 주제 번호에 해당하는 예시 헤드라인들의 내용을 통해 주제를 파악하세요.
- 텍스트에서 의미 있는 단어나 구문을 식별하세요.
- 식별된 키워드와 주제에 맞는 헤드라인으로 작성하세요.
...
def main():
    ...
    df = pd.read_csv("./resources/processed/train_noisy_ner.csv")
    ...
    df.to_csv("./resources/processed/train_noisy_ner_restore.csv",encoding='utf-8-sig')
